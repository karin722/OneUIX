...
> **この機能は中止しました。** 実機（ SM-S931Z / One UI 8.5 ）でタスクバー自体は表示できたものの、
> アイコンのサイズが 0 のままになる問題を最後まで解消できず、実装は削除してあります。
> 動く実装はこのリポジトリに入っていません。
>
> 以下は「何がどこで塞いでいるのか」を実機の APK とログから調べた記録で、
> 再挑戦するときの出発点として残す。記述内容は調査時点のもの。
